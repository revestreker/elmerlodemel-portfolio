...
(function () {
  ...
  function initCategory(listRoot) {
    ...
    projectsInView.forEach(function (project, projectIndex) {
      ...
      /* Walk the items in order. Runs of stills collect into one masonry
         block; media and featured items each get their own full-width block. */
      var gallery = null;
      var imageIndex = 0;
      ...
      project.items.forEach(function (item) {
        if (item.kind === 'image') {
          ...
          gallery.appendChild(buildShot(project, item, projectIndex, imageIndex));
          imageIndex++;
          return;
        }

        gallery = null;
        var block = buildMedia(item);
  ...
  function media(className, inner, caption) {
      ...
      case 'youtube':
        return media('feature-video',
          frame('https://www.youtube-nocookie.com/embed/' + item.id +
                (item.start ? '?start=' + item.start : ''), item.title),
          item.title);

      case 'speakerdeck': {
        var block = media('feature-deck',
          frame('https://speakerdeck.com/player/' + item.id, item.title, item.ratio || '710/297'));
        if (item.href) {
          var out = el('a', 'feature-link', 'Open on Speaker Deck →');
          out.href = item.href;
          out.target = '_blank';
          out.rel = 'noopener';
